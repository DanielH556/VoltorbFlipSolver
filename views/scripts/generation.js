(function createBoard(card) {
  ...
  var elementCnt = '';
  cards.forEach(card => {
    elementCnt += `<button class="card" type="button" id="card-${card.id}" ng-click="flipCard(${card.id},${card.state},${card.value})"></button>`;
  });
  ...
  labels.forEach(label => {
    ...
    <input type="number" name="sum-${label.id}" id="right-voltorb-card-${label.id}" ng-value=${label.voltorb} min="0" max="5"><img src="../views/medias/Voltorb.png" alt="LabelVoltorb" class="label-ref"></div>`;
    ...
    <input type="number" name="sum-${label.id}" id="bottom-voltorb-card-${label.id}" ng-value=${label.voltorb} min="0" max="5"><img src="../views/medias/Voltorb.png" alt="LabelVoltorb" class="label-ref"></div>`;
  });
